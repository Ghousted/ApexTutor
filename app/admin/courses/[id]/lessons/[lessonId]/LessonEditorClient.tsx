...
export default function LessonEditorClient({
  ...
  if (!lesson) {
    return (
      <div className="p-8">
        <p className="text-sm text-rose-600">{error || "Lesson not found"}</p>
        <Link
    ...
    <div className="px-6 md:px-10 py-8 max-w-3xl">
      ...
      <header className="mb-6 flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <textarea
            ...
            rows={2}
            className="w-full mt-1 text-sm text-slate-600 bg-transparent outline-none resize-none border-b border-transparent focus:border-slate-200"
          />
        ...
        <button
          ...
          className="shrink-0 p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-md"
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <button
          ...
          className="inline-flex items-center gap-1 px-2.5 py-1 text-[11px] font-medium text-indigo-700 bg-indigo-100 hover:bg-indigo-200 rounded-full transition-colors"
        >
        ...
        {pickerOpen ? (
          <div className="bg-white border border-slate-200 rounded-2xl p-3 shadow-sm">
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-semibold text-slate-600">Pick a step type</p>
              <button
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {STEP_TYPES.map((t) => (
                <button
                  ...
                  className="text-left p-2.5 rounded-xl border border-slate-200 hover:border-indigo-400 hover:bg-indigo-50 transition-colors"
                >
          ...
          <button
            ...
            className="w-full px-4 py-3 border-2 border-dashed border-slate-200 hover:border-indigo-400 hover:bg-indigo-50/50 rounded-2xl text-sm font-medium text-slate-600 hover:text-indigo-700 transition-all flex items-center justify-center gap-2"
          >
    ...
    >
      <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 border-b border-slate-100">
        <button
          {...attributes}
          {...listeners}
          className="p-1 text-slate-400 hover:text-ink cursor-grab"
          aria-label="Drag to reorder"
        >
          <GripVertical className="w-4 h-4" />
        </button>
        <span className="w-6 h-6 rounded-full bg-white border border-slate-200 text-xs font-semibold text-slate-600 flex items-center justify-center">
          {index + 1}
        </span>
        <span className="text-base">{meta.emoji}</span>
        <span className="text-sm font-semibold text-ink flex-1">{meta.label}</span>
        <button
