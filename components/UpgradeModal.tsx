...
export default function UpgradeModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="relative bg-coal rounded-[14px] shadow-2xl w-full max-w-2xl p-7 max-h-[90vh] overflow-y-auto">
        <button
          onClick={onClose}
          ...
          {plans.map((p) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "text-left rounded-[14px] p-5 transition-all border-2",
                  isSelected
                    ? p.highlight
                      ? "border-[var(--border-strong)] bg-iron/40"
                      : "border-ink bg-coal"
                    : "border-[var(--border-subtle)] hover:border-[var(--border-strong)]"
                )}
              >
                <div className="flex items-baseline justify-between mb-1">
                  <p className="text-xs font-semibold text-ash-gray uppercase tracking-wider">
                    {p.name}
                  </p>
                  {p.highlight && (
                    <span className="text-[10px] font-semibold text-canvas-white bg-iron rounded-full px-2 py-0.5">
                      RECOMMENDED
                    </span>
                  )}
                </div>
                <div className="flex items-baseline gap-1 mb-2">
                  <span className="text-2xl font-bold text-canvas-white">{p.price}</span>
                  <span className="text-xs text-ash-gray">/ month</span>
                </div>
                <p className="text-xs text-ash-gray mb-3">{p.blurb}</p>
                <ul className="flex flex-col gap-1.5">
                  {p.features.map((f) => (
                    <li key={f} className="flex items-start gap-1.5 text-xs text-canvas-white/90">
                      <Check
        ...
        {error && (
          <p className="text-sm text-canvas-white bg-coal border border-[var(--border-subtle)] rounded-lg px-3 py-2 mb-3">
            {error}
          </p>
        )}
        ...
        <p className="text-[11px] text-center text-ash-gray mt-3 leading-relaxed">
